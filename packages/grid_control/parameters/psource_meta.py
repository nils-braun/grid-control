...
def _sort_deps(psrc_list):
	metadata_list_all = []
	for psrc in psrc_list:
		psrc.fill_parameter_metadata(metadata_list_all)
	metadata_set_all = set(lmap(str, metadata_list_all))
	...
	while psrc_idx_list:
		...
		if psrc_idx not in psrc_idx_list:
			yield psrc
			psrc.fill_parameter_metadata(metadata_list_known)
			metadata_list_known = lmap(str, metadata_list_known)
